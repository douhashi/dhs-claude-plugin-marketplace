...
- マークダウン記法・コードブロック・URL は使わない。記号は語尾の表情絵文字のみ許可（むしろ必須）
...
文**であること（英文の音写は禁止）。**いずれも語尾は表情絵文字で終える**:
- `text`: 日本語の原文表記。漢字・数字・ひらがなを普通に使う。固有名詞・
  技術用語に限り英語表記が残ってよい。末尾に表情絵文字
- `reading`: text と同じ内容で、残った英単語（固有名詞・技術用語）のみ
  カタカナにした版。漢字・ひらがな・数字・既存のカタカナ語はそのまま
  （TTS が読み上げる）。末尾は text と**同じ絵文字**

例（語尾は絵文字、句点なし）:
  text:    "サーバ起動完了した😌"
  reading: "サーバ起動完了した😌"

  text:    "GitHub Actions 通った😎"
  reading: "ギットハブ アクションズ 通った😎"
...
各 sentence は `text` と `reading` の 2 フィールド。**どちらも自然な日本語の
文**であること（英文の音写は禁止）。**いずれも語尾は表情絵文字で終える**:
- `text`: 日本語の原文表記。固有名詞・技術用語に限り英語表記が残ってよい。末尾に表情絵文字
- `reading`: text と同じ内容で、残った英単語（固有名詞・技術用語）のみ
  カタカナにした版。漢字・ひらがな・数字・既存のカタカナ語はそのまま。末尾は text と**同じ絵文字**
