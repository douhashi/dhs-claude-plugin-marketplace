...
- **自走開発**: 前提条件（Infisical・環境変数）を検査してループ開発のコンテキストを `.tmp` に書き出すスキル（`spira:autopilot`）と、それに従い `spira:pick` → `spira:do` を最大 3 ラインで回し、ループ中に見つかったシステムを壊す不具合をロードマップに追加（PR→マージ）しながら表で進捗を報告するエージェント（`spira:orchestrator`）
