...
- **書き手**
  ...
  - orchestrator エージェント（追加: ループ中に見つかった、システムを壊す不具合を取り込むとき。前のイテレーションで未マージの取り込みも含める。手順 2-3）
  - orchestrator エージェント（整合修正: ループの終わりに、チェック状態や完了行の置き場所をループの結果に合わせるとき。NEXT が done / halt のイテレーションの手順 7）
...
- 前のイテレーションの未マージ PR を統合した場合は、表の下に `統合した PR: #<番号>` を 1 行足す
...
| 項目 | 書式 |
|:--|:--|
| ブランチ | キックオフ: `autopilot/roadmap-kickoff-sync` ／ ループ終了時: `autopilot/roadmap-final-sync-<イテレーション番号>` |
