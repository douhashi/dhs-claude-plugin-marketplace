...
- 既存行の文言は書き換えない。追加 PR は行の挿入だけ、整合修正 PR は「整合の規則」とループ固有の規則（orchestrator 手順 6）にある変更だけを行う
- 未完了行どうしの並べ替えはしない

## 整合の規則

Issue の状態だけで決まるロードマップのずれと、その直し方。autopilot（キックオフ）と orchestrator（手順 6）が共通に使う。
...
- Issue が Open の `[~]` の行は、人が進めている可能性があるので触らない（自走の対象 Issue の `[~]` だけは orchestrator が手順 6 で直す）
...
| 項目 | 書式 |
|:--|:--|
| ブランチ | キックオフ: `autopilot/roadmap-kickoff-sync` ／ ループ中: `autopilot/roadmap-sync-<イテレーション番号>` |
