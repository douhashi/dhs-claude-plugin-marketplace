...
- **書き手**
  - autopilot スキル（キックオフ整合: チェック状態や完了行の置き場所が Issue の状態とずれているとき。Phase 4）
...
- キックオフ整合・キックオフ整理・追加・整合修正・起票後の追記それぞれ、**1 本の PR にまとめる**
...
Issue の状態だけで決まるロードマップのずれと、その直し方。autopilot（キックオフ）と orchestrator（ループ終了時の手順 7）が共通に使う。
...
| 項目 | 書式 |
|:--|:--|
| ブランチ | キックオフ: `autopilot/roadmap-kickoff-sync` ／ ループ終了時: `autopilot/roadmap-final-sync-<見回り番号>` |
...
キックオフ整合・キックオフ整理・追加・整合修正・起票後の追記で共通に使う。`BR` はブランチ名、`TITLE` は PR タイトル、
