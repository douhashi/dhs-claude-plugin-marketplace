...
5. **状態はファイルに残す**。進捗は `state.md`、各ラインの出力は `lines/` にある。`lines/<Issue 番号>.log` は `claude -p` の stream-json で、経過がイベントごとに 1 行ずつ増え、最後の `"type":"result"` 行に結果・費用・ターン数・所要時間が入る。人が経過を追うときは `{{PLUGIN_ROOT}}/scripts/watch-lines.sh {{ROOT}}/.tmp/spira-autopilot/lines` を使う。各ラインは起動ごとに新しいセッション ID（SID）で走り、`lines/<Issue 番号>.sid` と `state.md` のライン表・結果表に残る。終わったラインは、autopilot と同じ `CLAUDE_CONFIG_DIR` で `cd {{ROOT}} && claude --resume <SID>` とすれば会話を開いて事後調査できる（worktree の再作成は不要）
