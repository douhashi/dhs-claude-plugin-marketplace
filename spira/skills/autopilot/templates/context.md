...
5. **状態はファイルに残す**。進捗は `state.md`、各ラインの出力は `lines/` にある。`lines/<Issue 番号>.log` は `claude -p` の stream-json で、経過がイベントごとに 1 行ずつ増え、最後の `"type":"result"` 行に結果・費用・ターン数・所要時間が入る。人が経過を追うときは `{{PLUGIN_ROOT}}/scripts/watch-lines.sh {{ROOT}}/.tmp/spira-autopilot/lines` を使う
