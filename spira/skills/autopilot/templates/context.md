...
7. **`escalated` Issue は自走では扱わない**（ループ開始前からあるものも含む）。ループ中に見つかっても、止めるのは自走を続けられないときだけ。他の Issue を進められるなら、見送って続ける
8. **ロードマップは常に現状を表す**。orchestrator は毎イテレーションの最後にロードマップとの整合を確かめ、ずれていればチェック状態を直す PR を作ってマージする
