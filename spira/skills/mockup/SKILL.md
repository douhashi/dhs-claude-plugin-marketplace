...
- 承認前に `docs/mockups/` と `docs/development/` のファイルを書き換えることは禁止（Phase 8 の承認が取れるまで Phase 9 に進まない）
...
Phase 4 のデザインスキルと Phase 9 の `spira:update-doc` は**同じセッションに指示が読み込まれる**（サブエージェントではない）。
...
| デザインシステムのドキュメント | `docs/development/design-system.md` | コードのトークンを探す |
| コードのトークン（デザインシステムのドキュメントが無いときだけ） | Tailwind の設定（`tailwind.config.*`・CSS の `@theme`）・CSS の `:root` の変数・UI ライブラリのテーマのファイル | Phase 5 で方向性を選ぶ |
| ビジュアライゼーションのドキュメント | `docs/development/visualization.md` | 動きのパターンかデータの見せ方を決めたら Phase 9 で新規に作る |
...
3. 次のうち文書から読み取れないものだけを、まとめて 1 回だけ質問する（最大 2 つ）
...
1. デザインシステムの出どころを次の表で決める

   | 状況 | 扱い |
   |:--|:--|
   | デザインシステムのドキュメントがある | 従う。そのトークンとコンポーネントで描けるかを確かめ、足りないものを拡張の候補として控える |
   | ドキュメントは無いが、コードにトークンがある | コードのトークンを抜き出し、トークンの 3 層に整理してデザインシステムの案にする（コードから起こす）。値は変えず、足りない層だけ補う |
   | どちらも無い | Phase 5 で方向性を 3 案から選んでもらう（新規に作る）。選ばれた方向性をデザインシステムの案にする |

2. どの場合も、動きのトークン（時間・イージング）と `prefers-reduced-motion` のときの扱いをデザインシステムの案に含める。既存のデザインシステムに無ければ拡張の候補にする
...
デザインスキルが入っていない環境では、同じ観点を自分の知識で確かめ、Phase 7 の提案にその旨を 1 行添える。
...
| ユーザーの応答 | 行動 |
|:--|:--|
| 1 案を選ぶ | その案をデザインシステムの案にして Phase 6 へ進む |
| 案を組み合わせる（例: A の配色で B の書体） | 組み合わせた方向性で主の画面を描き直して比較ページに足し、確かめてもらう |
| どれも合わない | 合わない点を聞き、3 案を作り直して公開し直す |
...
| 見送り | 反映をせずに Phase 10 へ進む |

**承認が取れるまで Phase 9 に進まない。**
...
1. Phase 9 の PR URL とマージ結果（Phase 9 を行わなかった場合は「反映なし」）
2. 反映したファイルの一覧
3. 提案ページの URL（Phase 5 を行った場合は方向性の比較ページの URL も）
