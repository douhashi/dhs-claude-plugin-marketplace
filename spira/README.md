...
spira は外部仕様の検証に **Context7 MCP** を必須とします。
reviewer エージェントは外部依存（ライブラリ・API 等）の使用方法を Context7 経由で公式ドキュメントから検証するため、Context7 が有効になっていない環境では正常に動作しません。

Context7 の導入方法は [Context7 公式サイト](https://context7.com) を参照してください。
