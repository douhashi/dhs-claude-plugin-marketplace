...
5. **モックアップの作成** — 画面ごとの HTML を書き、それを埋め込んだ提案ページ（画面・現状との比較・要素の根拠・デザインの判断・デザインシステム・動き・反映するファイル）を 1 つのアーティファクトとして公開する
6. **提案** — チャットには提案ページの URL と要約だけを示す
7. **承認** — 修正指示（チャットかページへのコメント）なら同じ URL に公開し直して再提示する
8. **反映** — `spira:update-doc` で `docs/mockups/` と、必要なら `docs/development/` のドキュメントを作成・更新し、マージまで行う
9. **完了報告** — PR URL・反映したファイル・提案ページの URL を報告
...
| 出力 | テンプレート |
|:--|:--|
| 提案ページ（アーティファクト）とチャットでの提示 | `proposal.md` |
